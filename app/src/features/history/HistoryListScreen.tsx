...
function HistorySessionRow({ session, routineName }: { session: WorkoutSession; routineName: string }) {
  ...
  return (
    <Card as={Link} to={`/history/${session.id}`} className="flex items-center gap-4 !py-3">
      <div
        className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl"
        style={{ background: 'var(--color-accent-muted)', color: 'var(--color-accent)' }}
        aria-hidden="true"
      >
        <iconify-icon icon="ph:barbell-fill" className="text-3xl" />
      </div>
      <div className="min-w-0 flex-1">
        <p className="truncate text-lg font-bold" style={{ color: 'var(--color-text)' }}>
          {routineName}
        </p>
        <p
          className="truncate text-[10px] font-bold uppercase tracking-widest mt-1"
          style={{ color: 'var(--color-text-muted)' }}
        >
          {formatDateTime(session.startedAt)} · {countsLabel}
        </p>
      </div>
